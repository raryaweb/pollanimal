...
// Total user credits
        ...
        foreach ($entries as $key => $value) {
            $entriesnew = CampaignResult::select('user_id')->where([['updated_at', '>=', Auth::guard('web')->user()->last_login], ['campaign_id', '=', $value->id]])->get();
...
//CampaignResult::where('campaign_id', '=', id )->count()
        ...
        ->orderBy('advertise_credits', 'desc')
        ->paginate(20);*/
        $anketos = DB::select('SELECT * FROM campaigns AS c where advertise_credits > (SELECT COUNT(*) FROM campaigns_results AS cr WHERE cr.campaign_id = c.id) AND public = 1 AND active = 1 ORDER BY advertise_credits DESC LIMIT 10');
    ...
    public function answers($id)
    {
        ...
        if ($entry) {
            if ($entry->respondents) {
                ...
                ->orderBy('advertise_credits', 'desc')
                ->paginate(20);*/
                $anketos = DB::select('SELECT * FROM campaigns AS c where advertise_credits > (SELECT COUNT(*) FROM campaigns_results AS cr WHERE cr.campaign_id = c.id) AND public = 1 AND active = 1 ORDER BY advertise_credits DESC LIMIT 20');
